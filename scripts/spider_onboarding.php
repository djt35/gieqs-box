...
//ADD SEND MAIL TO OTHER FILE IF < 3 DAYS !!!
        ...
        echo ' Aborting 3 day script';

        //die();
    }
        
//remove previous programmes array
$programmes = false;
...
if ($programmes){ //there is a course today
...
if (($currentNavTime > $desiredMailSendTimeOnCourseDateMinus1) && ($currentNavTime < $desiredMailSendTimeOnCourseDatePlus1)){
   //if current time is +/- 1 hour from 0700 on 3 days before course, execute the mail
        ...
        //set some basics FOR THE EMAIL
        $filename = '/assets/email/subscriptions/onboarding_course_Zoom_today.php';
        $email_id = 'onboarding_course_' . $assetid; //unique id
        $subject = $assets_paid->getname() . ' joining instructions';
        $preheader = 'Reminder : GIEQs Online Course - ' . $assets_paid->getname() . ' is today!';
        //$page = BASE_URL . '/pages/learning/pages/account/billing.php?showresult=' . $subscription_id;
        ...
        foreach ($population as $key=>$value){
            ...
            echo 'there is a course today (' . $courseDateFull->format('d/m/Y') . ') but it is not correct time to send mail (Current time is : ' . $currentNavTime->format('d/m/Y H:i') . ' CET), parameters were (start send : ' . $desiredMailSendTimeOnCourseDateMinus1->format('d/m/Y H:i') . ' CET to end send : ' . $desiredMailSendTimeOnCourseDatePlus1->format('d/m/Y H:i') . ' CET)';
        ...
        echo 'No course data today  (' . $courseDate->format('d/m/Y') . ')';
