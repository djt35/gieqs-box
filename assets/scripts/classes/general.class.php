...
class general {
	...
	public function getDvidDetail($id)
	{
	...
	public function getSecretDetails($cretype)
	{
		

		$q = "SELECT * FROM `credential` WHERE `cre_type` = '$cretype'";
		$result = $this->connection->RunQuery($q);
		$details = $result->fetch_array(MYSQLI_ASSOC);
		return $details;
	}
	public function getgoogleSecretDetails()
	{
		$q = "SELECT `token_value` FROM `jsontable` WHERE `token_key` = 'credential'";
		$result = $this->connection->RunQuery($q);
		$details = $result->fetch_array(MYSQLI_ASSOC);
		
		$str = trim($details['token_value']);

		$str = str_replace('&quot;','"',$str);

		return $str;

		// $q = "SELECT * FROM `credential` WHERE `cre_type` = 'google'";
		// $result = $this->connection->RunQuery($q);
		// $details = $result->fetch_array(MYSQLI_ASSOC);
		// return $details;
	}
	public function gettokennDetails()
	{
		$q = "SELECT `token_value` FROM `jsontable` WHERE `token_key` = 'token'";
		$result = $this->connection->RunQuery($q);
		$details = $result->fetch_array(MYSQLI_ASSOC);
		return $details['token_value'];

		// $q = "SELECT * FROM `credential` WHERE `cre_type` = 'token'";
		// $result = $this->connection->RunQuery($q);
		// $details = $result->fetch_array(MYSQLI_ASSOC);
		// return $details;
	}
	public function updateClientAccessToken($clientToken)
	{
		$q = "UPDATE `jsontable` SET `token_value`='$clientToken' WHERE `token_key`='token'";
